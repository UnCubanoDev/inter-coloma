...
export function PlayoffAdmin({ match }: { match: PlayoffRow }) {
  ...
  return (
    <tr className="border-t border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
      ...
      <td className={`px-4 py-3 text-sm ${isReady ? 'font-medium' : 'text-gray-400'}`}>{isReady ? <TeamBadge numero={match.equipoLocal!.numero} name={match.equipoLocal!.nombre} /> : localName}</td>
      ...
      <td className={`px-4 py-3 text-sm text-right ${isReady ? 'font-medium' : 'text-gray-400'}`}>{isReady ? <TeamBadge numero={match.equipoVisitante!.numero} name={match.equipoVisitante!.nombre} reverse /> : visitName}</td>
      <td className="px-4 py-3 text-center">
